...
class URLPaths_1stpay
{
	...
	const URL_API_VALIDATION = 'https://secure-v.goemerchant.com/secure/RestGW/Gateway/Transaction/';
        const URL_TRANS_CENTER_SUPPORT = 'http://support.1stpaygateway.net/transaction-center.aspx';
        const URL_GATEWAY_OPTIONS_SUPPORT = "http://support.1stpaygateway.net/transaction-center.aspx?article=gateway-options";
        const URL_SUBMIT_CC_BATCH_SUPPORT = "http://support.1stpaygateway.net/transaction-center.aspx?article=submit-credit-card-batch";
}

/**
 * Container for front-end messages.
 */
class Firstpay_messages
{
    const WC_GATEWAY_ID = "1stpay";
    const WC_GATEWAY_TITLE = "1stPayGateway";
    
    const MSG_AUTH_APPROVED = "Your payment has been approved and is being processed.";
    const DESC_METHOD = "Process transactions using 1stPayGateway. Click <a href='http://support.1stpaygateway.net/transaction-center.aspx'>here</a> to visit our support page for details on viewing transaction history, issuing refunds, and more.";
    ...
    const DESC_GATEWAY_ID = "You can find your gateway and processor ID by logging into the transaction center and following the steps listed <a href='http://support.1stpaygateway.ney/transaction-center.aspx?article=gateway-options'>here</a>.";
    
    const TITLE_PROCESSOR_ID = 'Processor ID';
    
    const LABEL_AUTH_ONLY = "If enabled, you must manually submit transactions for settlement in your Transaction Center in order to capture the funds. Visit our <a href='http://support.1stpaygateway.ney/transaction-center.aspx?article=submit-credit-card-batch'>support page</a> for a walkthrough of settling transactions.";
    ...
    const DESC_ORDER_PREFIX = "Text to prepend to the WooCommerce order number. Can be used to distinguish orders from different WooCommerce sites processing through the same 1stPayGateway account. Only visible within the Transaction Center.";
...
class WC_Gateway_1stpay extends WC_Payment_Gateway_CC {
    ...
    static $apiURL = URLPaths_1stpay::URL_API; //Base URL for REST API requests
    ...
    function __construct() {
        ...
        $this->id                 = Firstpay_messages::WC_GATEWAY_ID;
        $this->icon               = false;
        ...
        $this->method_title       = __(Firstpay_messages::WC_GATEWAY_TITLE, 'wc-1stpay' );
        $this->method_description =
                __(Firstpay_messages::DESC_METHOD, 'wc-1stpay' );
        ...
        $this->title              = empty( $title ) ? __(Firstpay_messages::WC_GATEWAY_TITLE, 'wc-1stpay' ) : $title;
        $this->description        = $this->get_option( 'description' );
        $this->instructions       = $this->get_option( 'instructions', $this->description );
        self::$apiURL             = $this->get_option( 'sandbox' ) == 'yes' ? URLPaths_1stpay::URL_API_VALIDATION : URLPaths_1stpay::URL_API;
        ...
        add_action( 'update_option', array( $this, 'validate_1stpay_option'), 10, 3);
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                'title' => __(Firstpay_messages::TITLE_ENABLED, 'wc-1stpay'),
                'type' => 'checkbox',
                'label' => __(Firstpay_messages::LABEL_ENABLED, 'wc-1stpay'),
                'default' => 'no'
            ),
            'sandbox' => array(
                'title' => __(Firstpay_messages::TITLE_SANDBOX, 'wc-1stpay'),
                'type' => 'checkbox',
                'label' => __(Firstpay_messages::LABEL_SANDBOX, 'wc-1stpay'),
                'default' => 'no'
        ));
        if (class_exists("WC_Subscription")) {
            $this->form_fields['auto-renew'] = array(
                'title' => __(Firstpay_messages::TITLE_AUTO_RENEW, 'wc-1stpay'),
                'type' => 'checkbox',
                'label' => __(Firstpay_messages::LABEL_AUTO_RENEW, 'wc-1stpay'),
                'default' => 'yes');
        }
        $this->form_fields['payment-method-title'] = array(
            'title' => __(Firstpay_messages::TITLE_PAYMENT_METHOD_TITLE, 'wc-1stpay'),
            'type' => 'text',
            'description' => __(Firstpay_messages::DESC_PAYMENT_METHOD_TITLE, 'wc-1stpay'),
            'default' => __(Firstpay_messages::DEFAULT_PAYMENT_METHOD_TITLE, 'wc-1stpay'),
        );
        $this->form_fields['instructions'] = array(
            'title' => __(Firstpay_messages::TITLE_INSTRUCTIONS, 'wc-1stpay'),
            'type' => 'textarea',
            'description' => __(Firstpay_messages::DESC_INSTRUCTIONS, 'wc-1stpay'),
            'default' => __(Firstpay_messages::DEFAULT_INSTRUCTIONS, 'wc-1stpay'),
        );
        $this->form_fields['gateway-id'] = array(
            'title' => __(Firstpay_messages::TITLE_GATEWAY_ID, 'wc-1stpay'),
            'type' => 'text',
            'description' => Firstpay_messages::DESC_GATEWAY_ID
        );
        $this->form_fields['processor-id'] = array(
            'title' => __(Firstpay_messages::TITLE_PROCESSOR_ID, 'wc-1stpay'),
            'type' => 'text'
        );
        $this->form_fields['auth-only'] = array(
            'title' => __(Firstpay_messages::TITLE_AUTH_ONLY, 'wc-1stpay'),
            'type' => 'checkbox',
            'label' => __(Firstpay_messages::LABEL_AUTH_ONLY, 'wc-1stpay'),
            'default' => 'no'
        );
        $this->form_fields['vault-key-prefix'] = array(
            'title' => __(Firstpay_messages::TITLE_VAULT_KEY_PREFIX, 'wc-1stpay'),
            'type' => 'text',
            'description' => __(Firstpay_messages::DESC_VAULT_KEY_PREFIX, 'wc-1stpay'),
            'default' => Firstpay_messages::DEFAULT_VAULT_KEY_PREFIX
        );
        $this->form_fields['order-prefix'] = array(
            'title' => __(Firstpay_messages::TITLE_ORDER_PREFIX, 'wc-1stpay'),
            'type' => 'text',
            'description' => __(Firstpay_messages::DESC_ORDER_PREFIX, 'wc-1stpay'),
            'default' => Firstpay_messages::DEFAULT_ORDER_PREFIX
        );
    }
    ...
    public function print_admin_error($print = false) {
        ...
        $message = __(Firstpay_messages::PLEASE_ENTER_ID, 'wc-1stpay');
     ...
     * @param type $value
     */
    public function validate_1stpay_option($option, $old_value, $value) {
    ...
    public function process_payment($order_id) {
        ...
        if ($rgw->Result['isSuccess']) {
            ...
            wc_add_notice(Firstpay_messages::MSG_AUTH_APPROVED, 'success');
            $order->add_order_note(Firstpay_messages::MSG_AUTH_APPROVED);
            ...
            wc_add_notice(Firstpay_messages::ERR_PROBLEM_PROCESSING, "error");
            return;
        }
    }
    ...
    public function process_refund( $order_id, $amount = null, $reason = '') {
        ...
        if ($this->get_error_string($rgw)) { // credit failed, try void
            if ($amount != $order->get_total()) { // disallow partial voids
               return new WP_Error( 'partialVoidError', Firstpay_messages::ERR_PARTIAL_VOID );
            }
            else {
    ...
    function add_missing_fields_notice($printNotice = false) {
        if ($printNotice) {
            wc_print_notice(__('Payment error: ', 'woothemes') .
                    Firstpay_messages::ERR_MISSING_FIELDS, 'error');
        }
        else {
        ...
        if ($result["isError"] == TRUE) {
            foreach ($result["errorMessages"] as $index => $err) {
                if ($err == "Credit card account already exists") {
                    if (is_account_page()) {
                        wc_print_notice(Firstpay_messages::MSG_CARD_ALREADY_EXISTS, 'notice');
                    } elseif($isPaymentChange) {
                        wc_add_notice(Firstpay_messages::MSG_CARD_ALREADY_EXISTS, 'error');
                    }
                    else {
                        wc_add_notice(Firstpay_messages::MSG_CARD_ALREADY_EXISTS, 'notice');
                    }
                } else {
                    if (is_account_page()) {
            ...
            if (is_account_page()) {
                wc_print_notice(Firstpay_messages::MSG_PAYMENT_METHOD_SAVED, 'success');
            } else {
                wc_add_notice(Firstpay_messages::MSG_PAYMENT_METHOD_SAVED, 'success');
            }
    ...
    function get_merchant_info() {
        // Get merchant info from woocommerce -> settings -> checkout -> 1stpay
        ...
        if (!$this->is_valid_GUID($this->gateway_id) || 
            ...
            throw new InvalidInputException_1stpay(Firstpay_messages::ERR_INVALID_ID);
        }
    ...
    function get_card_number() {
        ...
        if (
            ...
            throw new InvalidInputException_1stpay(Firstpay_messages::ERR_CARD_NUMBER_INVALID);
        }
        else {
            return $cardNumber;
        }
     ...
     * @throws InvalidInputException_1stpay
     */
    function get_card_expiry() {
        ...
        if (strlen( $cardExpiry ) == 7 || strlen( $cardExpiry ) == 9) {
            ...
            throw new InvalidInputException_1stpay(Firstpay_messages::ERR_EXPIRY_INVALID_FORMAT);
        }
        ...
        if (
            ...
            throw new InvalidInputException_1stpay(Firstpay_messages::ERR_CARD_EXPIRY_INVALID);
        }
        else {
            return array (
    ...
    function get_card_cvc($forSavedCard) {
        ...
        if (
            ...
            throw new InvalidInputException_1stpay(Firstpay_messages::ERR_CARD_CVC_INVALID);
        }
        else {
            return $cvc;
        }
        ...
        if (is_user_logged_in() && !is_account_page()) {
            ...
            if (class_exists("WC_Subscription")) {
                $sub_msg = $this->get_option('auto-renew') == 'yes' ?
                        Firstpay_messages::MSG_AUTO_RENEW :
                        "";
            }
    ...
    function get_error_string($restGateway){
        ...
        if ($result["isError"] == TRUE) {
            ...
            foreach ($result["errorMessages"] as $index => $err) {
                switch ($err) {
                    ...
                    case 'Request Denied' :
                    case 'Invalid Card' :
                    case 'CVD Data Error' : $errorString .= Firstpay_messages::ERR_CARD_DECLINED; break;
                    case 'CVC2/CID ERROR' :
                    ...
                    case 'CB Not Allowed' :
                    case 'AX Not Allowed' :
                        $errorString .= Firstpay_messages::ERR_BAD_CARD;
                        $errorString .= Firstpay_messages::ERR_TRY_DIFFERENT_CARD; break;
        ...
        if ($result['validationHasFailed']) {
            $errorString .= Firstpay_messages::ERR_PLEASE_CORRECT . "<br>";
    ...
    public function on_my_account_load() {
        ...
        } catch (InvalidInputException_1stpay $exception) {
    ...
    function get_selected_card_id() {
        ...
        if ($vaultId == 'DEFAULT') {
            throw new InvalidInputException_1stpay(Firstpay_messages::PLEASE_CHOOSE_CARD);
        }
        elseif (!ctype_digit($vaultId) ||
                $vaultId <= 0
                ) {
            throw new InvalidInputException_1stpay(Firstpay_messages::ERR_CARD_SELECT_INVALID);
        }
        else {
            return $vaultId;
        }
    ...
    function get_delete_card_id() {
        ...
        if (ctype_digit($vaultId)) {
            ...
            throw new InvalidInputException_1stpay(Firstpay_messages::ERR_CARD_DELETE_INVALID);
        }
    }
